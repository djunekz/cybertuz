...
tampil_skor() {
    ...
    echo ""
    echo -e "${CYAN}  ----------------------------------------------------------------${RESET}"
    echo -e "  $CT_SCORE: ${WHITE}${BOLD}$sc / $tot${RESET}  ($persen%)"
...
tampil_progress() {
    ...
    cek() {
        sudah_selesai "$1" && echo -e "  ${GREEN}[$CT_PASSED]${RESET}" || echo -e "  ${RED}[$CT_PENDING]${RESET}"
    }
...
latihan_menu() {
    while true; do
        ...
        sudah_selesai "N1" && tag="${GREEN}[$CT_PASSED]${RESET}" || tag="${DIM}[$CT_PENDING]${RESET}"
        echo -e "  ${GREEN}[1]${RESET} Level 1 : Mengenal Jaringan & Port              $tag"
        sudah_selesai "N2" && tag="${GREEN}[$CT_PASSED]${RESET}" || tag="${DIM}[$CT_PENDING]${RESET}"
        echo -e "  ${GREEN}[2]${RESET} Level 2 : Reconnaissance & OSINT Dasar          $tag"
        echo ""
        echo -e "${GREEN}  -- INTERMEDIATE --${RESET}"
        sudah_selesai "I1" && tag="${GREEN}[$CT_PASSED]${RESET}" || tag="${DIM}[$CT_PENDING]${RESET}"
        echo -e "  ${GREEN}[3]${RESET} Level 3 : Web Application Vulnerability         $tag"
        sudah_selesai "I2" && tag="${GREEN}[$CT_PASSED]${RESET}" || tag="${DIM}[$CT_PENDING]${RESET}"
        echo -e "  ${GREEN}[4]${RESET} Level 4 : Linux Security & Privilege Escalation $tag"
        echo ""
        echo -e "${GREEN}  -- ADVANCED --${RESET}"
        sudah_selesai "A1" && tag="${GREEN}[$CT_PASSED]${RESET}" || tag="${DIM}[$CT_PENDING]${RESET}"
        echo -e "  ${GREEN}[5]${RESET} Level 5 : Full Pentest Scenario - Web App       $tag"
        sudah_selesai "A2" && tag="${GREEN}[$CT_PASSED]${RESET}" || tag="${DIM}[$CT_PENDING]${RESET}"
        echo -e "  ${GREEN}[6]${RESET} Level 6 : Network Attack Simulation             $tag"
        echo ""
        echo -e "${GREEN}  -- EXPERT --${RESET}"
        sudah_selesai "E1" && tag="${GREEN}[$CT_PASSED]${RESET}" || tag="${DIM}[$CT_PENDING]${RESET}"
