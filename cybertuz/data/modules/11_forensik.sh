...
teori_forensik() {
    ...
    echo -e "${CYAN}  =================================================================${RESET}"
    echo ""
    echo -e "${WHITE}  $CT_C_FORENSIC_DEF${RESET}"
    
    echo ""
    echo -e "${GREEN}  $CT_C_FORENSIC_PRINCIPLES${RESET}"
    echo ""
    echo -e "${YELLOW}  $CT_C_F_PRESERVE${RESET}"
...
log_analysis() {
    ...
    echo -e "${CYAN}  =================================================================${RESET}"
    echo ""
    echo -e "${WHITE}  $CT_C_LOG_DEF${RESET}"
    
    echo ""
    echo -e "${GREEN}  $CT_C_LOG_LOCATIONS${RESET}"
    ...
    echo ""
    echo -e "${GREEN}  $CT_C_LOG_COMMANDS${RESET}"
    echo ""
    echo -e "${YELLOW}  $CT_C_LOG_FAIL${RESET}"
    echo -e "${WHITE}  grep 'Failed password' /var/log/auth.log | tail -20${RESET}"
    echo ""
    echo -e "${YELLOW}  $CT_C_LOG_SUCCESS${RESET}"
    echo -e "${WHITE}  grep 'Accepted password' /var/log/auth.log | tail -20${RESET}"
    echo ""
    echo -e "${YELLOW}  $CT_C_LOG_SUDO${RESET}"
    echo -e "${WHITE}  grep 'sudo' /var/log/auth.log | tail -20${RESET}"
    echo ""
    echo -e "${YELLOW}  $CT_C_LOG_APACHE${RESET}"
    echo -e "${WHITE}  awk '{print \$1}' /var/log/apache2/access.log | sort | uniq -c | sort -rn | head -10${RESET}"
    echo ""
    echo -e "${GREEN}  $CT_C_LOG_LOCAL${RESET}"
    echo ""
    echo -e "${CYAN}  $CT_C_LAST_CMDS${RESET}"
    history 2>/dev/null | tail -15 || cat ~/.bash_history 2>/dev/null | tail -15 || echo -e "${WHITE}  $CT_C_LOG_NONE${RESET}"
    echo ""
    echo -e "${CYAN}  $CT_C_LOGGED_USERS${RESET}"
...
file_analysis() {
    ...
    echo ""
    echo -e "${GREEN}  $CT_C_FILE_PRACTICE${RESET}"
    echo -ne "${WHITE}  $CT_C_ENTER_FILE${RESET}"
    read -r file_path
    if [[ -n "$file_path" && -f "$file_path" ]]; then
        echo ""
        echo -e "${CYAN}  $CT_C_FILE_INFO${RESET}"
        stat "$file_path" 2>/dev/null
        echo ""
        echo -e "${CYAN}  $CT_C_FILE_TYPE${RESET}"
        file "$file_path" 2>/dev/null
        echo ""
        echo -e "${CYAN}  $CT_C_HASH_MD5${RESET}"
        md5sum "$file_path" 2>/dev/null
        echo -e "${CYAN}  $CT_C_HASH_SHA256${RESET}"
        sha256sum "$file_path" 2>/dev/null
    else
        echo -e "${RED}  $CT_C_FILE_NOT_FOUND${RESET}"
    fi
    press_enter
}
